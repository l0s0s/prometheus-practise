...
func TestReadResponseStatusCode_RequestToUnauthorizedEndpoint_increaseStatus401Metric(t *testing.T) {
	ts := startTestServer()

	defer ts.Close()

	_, err := helper.MakeRequest(ts.URL + "/api/v1/mock/unauthorized")
	require.NoError(t, err)
